...
class Hangman(HANGMAN,WORDS) :
    ...
    def __init__(self, word_to_guess):
        ...
        self.game_progress = list('-'*len(self.word_to_guess)) # 
    ...
    def find_indexes(self, letter):
        ...
        """
        return [[i,char] for i, char in enumerate(self.word_to_guess) if i in letter]
    ...
    def is_invalid_letter(self, input_):
        ...
        """
        return (not input_isalpha()) or (input_.isalpha() and len(input_) >= 1) #input_이 숫자이거나, (글자,길이가 1 이상일 때) True
    ...
    def play(self):
        ...
        """
        while True:
            self.print_game_progress() # 현재 게임 상태를 먼저 출력해줌. 
            user_input = self.get_user_input # user_input 을 입력받는 메소드 적용. 

            # Validate the user input
            if not self.is_invalid_letter : # 플레이어가 입력한 인풋이 유효한 값인지 아닌지. 
            ...
            if self.user_input in :
                print('이미 입력한 알파벳입니다.') # 이미 입력받았던 알파벳이면, 이미 입력했다 메시지 출력
                continue    # break or continue 중 하나를 사용하여 계속 입력

            # when the user_input is a character in word_to_guess
            if _:
                indexes =   # 추측한 글자의 인덱스 받기 
                self.update_game_progress() # 입력받은 글자로 현재 게임 상태 업데이트 
